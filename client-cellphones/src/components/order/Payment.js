...
export default function Payment() {
  ...
  const { order } = useSelector((state) => state.orderInfo); // Kiểm tra order từ Redux store

  const VND_TO_USD_RATE = 0.000042; // Tỷ giá cố định từ VND sang USD

  // Kiểm tra và lấy tổng tiền của đơn hàng
  const totalPrice = order ? order.totalPrice : 0; // Kiểm tra order có tồn tại không
  const totalPriceInUSD = totalPrice * VND_TO_USD_RATE;
        ...
        <button type="submit" className="paypal">
          ...
          {sdkReady && (
            <PayPalButton
              ...
              amount={totalPriceInUSD} // Gửi số tiền đã chuyển đổi sang USD
